...
namespace LmuRu {
 ...
 public sealed class NativeState {
  public int Originals,Installed; public bool CanRestore;
  public string State {get{return Installed==0?"original":Originals==0?"installed":"partial";}}
 }
 public sealed class NativePackage {
  ...
  public NativeState Inspect(string root) {
   ...
   foreach(var f in Info.files) {
    ...
    if(hash==f.before_sha256)s.Originals++;else if(hash==f.after_sha256)s.Installed++;else throw new InvalidDataException("Неизвестная версия ресурса HUD: "+f.entry);
    if(File.Exists(backup)) {
     if(Package.HashFile(backup)!=f.before_sha256)throw new InvalidDataException("Повреждена резервная копия HUD: "+f.entry);
    }else if(hash==f.after_sha256)s.CanRestore=false;
   }
   return s;
  }
