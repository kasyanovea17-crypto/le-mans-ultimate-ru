...
$sources=@((Join-Path $root 'src\Engine.cs'),(Join-Path $root 'src\NativePatch.cs'),(Join-Path $root 'src\Updates.cs'),(Join-Path $root 'src\Launcher.cs'),(Join-Path $build 'BuildInfo.cs'))
...
if($Test){
 & $csc /nologo /target:exe /platform:x64 /main:LmuRu.UpdateTests ('/resource:'+$payload+',LMU.Payload') @refs ('/out:'+(Join-Path $build 'UpdateTests.exe')) @sources (Join-Path $root 'tests\UpdateTests.cs')
 if($LASTEXITCODE -ne 0){throw 'Update tests compilation failed'}
 & (Join-Path $build 'UpdateTests.exe')
 if($LASTEXITCODE -ne 0){throw 'Update tests failed'}
}
Write-Output 'BUILD_PASS launcher=1.1.0 payload=embedded dependencies=NET_Framework'
