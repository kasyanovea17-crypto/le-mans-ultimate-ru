...
namespace LmuRu {
 public static class NativeTests {
  ...
  static void Unit() {
   ...
   byte[] previous=Encoding.UTF8.GetBytes("previous translated version");list[0].previous_sha256=new[]{Package.Hash(previous)};File.WriteAllBytes(Path.Combine(root,list[0].entry),previous);Check(p.Inspect(root).Previous==1,"previous version detected");p.Apply(root,true,guard,progress);Check(p.Inspect(root).State=="installed","previous version upgraded");
